...
// Converted from a client component (lightbox state, no useTilt at all) to a
// plain Server Component: this destination teaser sits lower down a long
// homepage and its 8 photos are all visible at once already — a full-screen
// lightbox with keyboard/arrow navigation, wired identically to the main
// Gallery, was disproportionate for a section nobody needed to "browse".
// All 8 photos, every alt text and every line of copy (eyebrow/title/text/
// ebike_note/winter_caption) are unchanged — only the click-to-zoom
// interaction is gone, so nothing indexable here is any different.
export default async function Summer({ locale }: { locale: Locale }) {
  const t = await getTranslations({ locale, namespace: 'summer' });

  const renderGrid = (photos: typeof images) =>
    photos.map((img, i) => (
  ...
  return (
    <section id="estate" className="bg-mist py-24 md:py-32">
      <div className="max-w-content mx-auto px-6 md:px-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{renderGrid(images)}</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{renderGrid(winterImages)}</div>
      </div>
    </section>
  );
}
