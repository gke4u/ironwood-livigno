...
- **Pagine satellite isolate dalla homepage (finding corretto e risolto in Fase 1)**: `/inverno`, `/estate`, `/famiglie`, `/benessere`, `/come-arrivare`, `/chi-siamo`, `/sauna-bagno-turco-privato-livigno`, `/camere-appartamento-livigno`, `/livigno-estate` — priorità 0.8 in sitemap. La formulazione originale di questo finding era imprecisa: **`Footer.tsx` linkava già tutte le 9 pagine dall'homepage `/it`** (array `itLandingPages`, presente da prima dell'audit, con link intenzionalmente omessi sulle altre 11 lingue perché il contenuto è IT-only). Il gap reale era solo nella **Nav** (barra di navigazione principale, desktop e mobile), dove le pagine satellite non comparivano affatto. Risolto aggiungendo un menu a tendina "Scopri" nella Nav desktop (`Nav.tsx`, tra "Posizione" e "Blog") e la stessa lista nel menu mobile (`MobileMenu.tsx`), entrambi visibili solo per `locale === 'it'` — stessa logica già usata dal footer. Lista delle 9 pagine estratta in `src/data/satellite-pages.ts`, condivisa da Footer/Nav/MobileMenu per evitare tripla duplicazione. Build verificata pulita (97/97 pagine); confermato via grep sull'export che "Scopri" e i 9 link compaiono in `out/it.html` e non in `out/en.html`.
